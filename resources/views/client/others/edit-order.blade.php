...
@section('meta-title')
	...
	<title>{{trans('general.edit-order')}} | {{trans('general.new-other-order')}}</title>
...
@section('content')
				...
				<div class="col-xs-12">
					<div class="page-header-wrap {{(App::getLocale() === 'ar') ? 'breadcrumb-right':''}}">
						...
						<ol class="breadcrumb">
							...
							<li class="active">{{trans('general.edit-order')}} </li>
						</ol>
					</div>
				</div>
			</div>
	...
	<section class="section-padding">
		<div class="container">
			<div class="row">
				<h3 class="text-center text-white" style="background: #31708f; margin: 10px 50px 0px 50px;">{{trans('general.edit-order')}}</h3>
				<div class="col-xs-12 {{(App::getLocale() === 'ar') ? 'text-right':''}}" style="border: 1px solid #31708f; padding: 30px;">
					...
					@endif
					<form method="post" action="{{route('client.other-edit-order',array(App::getLocale(),$order->ref_number))}}"  class="row">
						{{csrf_field()}}
						...
						<div class="form-section">
							<div class="col-sm-12">
								...
								<div class="form-group col-xs-12 col-sm-3  form-inline">
									<label><input id="photo" type="checkbox" name="photo" value="1" {{($order->is_photo = 1)?'checked="true"':''}}>&nbsp;{{trans('general.photography')}}</label>
								</div>
								...
								<div class="form-group col-xs-12 col-sm-4">
									...
									<select class="form-control" name="PhotoAddress" id="p_address" {{($order->is_photo = 1)?'':'disabled="disabled"'}}>
